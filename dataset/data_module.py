...
def convert_raw_forget_data_to_model_format(tokenizer, max_length, question, answer, model_configs, mask=True):
    ...
    # print(f"num_question_tokens:{num_question_tokens}")
    ...
    # print(f"len of encoded.input_ids:{len(encoded.input_ids)}")
    ...
    if mask:
        ...
        for i in range(num_question_tokens): 
            # 当max_length太小时，label 全部内容都被mask, 导致answer部分的内容未能输入
            if i < len(label):
                label[i] = -100
    else:
        label = pad_input_ids
...
# for ME + GD loss
class SimpleTextForgetDatasetQA(Dataset):
    """只处理forget和retain数据"""
    def __init__(self, tokenizer, model_family, forget_data, retain_data, max_length=512, mask=True):
        super(SimpleTextForgetDatasetQA, self).__init__()
    ...
    def __getitem__(self, idx):
        ...
        for data_type in self.data_types:
            if data_type == "retain":
                ...
                question = data[retain_idx]["conversations"][0]["value"]
                answer = data[retain_idx]["conversations"][1]["value"]

            else:  # forget
                data = self.forget_data
                question = data[idx]["conversations"][0]["value"]
                answer = data[idx]["conversations"][1]["value"]

            if data_type == 'forget':
                converted_data = convert_raw_forget_data_to_model_format(
                    self.tokenizer, self.max_length, question, answer, 
                    self.model_configs, mask=self.mask
                )
            else:  # retain
                converted_data = convert_raw_forget_data_to_model_format(
                    self.tokenizer, self.max_length, question, answer, 
                    self.model_configs, mask=True
                )
...
# for IDK + AP loss
class IDKTextForgetDatasetQA(Dataset):
    """处理forget_IDK, retain, retain_idk"""
    
    def __init__(self, tokenizer, model_family, forget_data, retain_data, max_length=512, mask=True):
        super(IDKTextForgetDatasetQA, self).__init__()
        self.tokenizer = tokenizer
        self.max_length = max_length
        self.forget_data = forget_data
        self.retain_data = retain_data
        ...
        self.data_types = ["retain", "forget_idk", "retain_idk"]
...
def simple_data_collator_forget(samples):
    """简化版forget数据整理函数：只处理forget和retain"""
    rets = []

    # 只提取forget和retain两种数据类型
...
def idk_data_collator_forget(samples):
    """简化版forget数据整理函数：只处理forget和retain"""
    rets = []

    data_types = ["retain", "forget_idk", "retain_idk"]
    samples_dict = {data_type: [sample[i] for sample in samples] for i, data_type in enumerate(data_types)}
